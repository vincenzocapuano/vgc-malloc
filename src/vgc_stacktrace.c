...
// stack trace
//
static void stacktrace(ProcessPath *self, char **messages, int size)
{
	...
	for (int i = 2; i < size - 1; i++) {
		...
		*end = 0;
		...
		char addr2line[BUFFER_SIZE];
		snprintf(addr2line, BUFFER_SIZE, "addr2line -Cfipe %s%s", strncmp(messages[i], self->base, strlen(self->base)) == 0 ? (*self->base = 0, self->path) : "", messages[i]);
...
void vgc_stacktraceShow(VGC_mallocHeader *mallocBlock)
{
	...
	stacktrace(getProcessPath(), messages, mallocBlock->btArraySize);
	free(messages);
#endif
}
...
static void vgc_stacktrace(int sigNumber, siginfo_t *info, void *ucontext)
{
	...
	stacktrace(getProcessPath(), messages, size);
	free(messages);
}
#endif
